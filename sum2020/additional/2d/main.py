...
from calculation import split_on_clusters, get_grid, NOT_CHECKED, drop_cluster_lower_than, \
    calculate_clusters_diameters, drop_not_conductive_clusters_dict
from visualize import draw_array_as_grid, map_dict_on_grid, EMPTY
...
lol = init_grid
lol = np.where(lol==np.max(init_grid), -1111, lol)
lol = np.where(lol==np.min(init_grid), np.max(init_grid), lol)
lol = np.where(lol==-1111, np.min(init_grid), lol)

draw_array_as_grid("f1", lol)
